...
GeneratorServantH::doHome(IR__::HomeDef_ptr home)
{
	...
	//
	if (already_done_.find(home->id()) != already_done_.end())
	{
		return;
	}
	...
	std::string header_name = filename_;
	filename_.append(".h");
	...
	open_module(out, component_, "Servants_");
	out << "\n\n";

	doComponent(component_);
