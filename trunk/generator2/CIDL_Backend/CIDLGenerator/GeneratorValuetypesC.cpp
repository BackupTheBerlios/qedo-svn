...
GeneratorValuetypesC::doValue(IR__::ValueDef_ptr value)
{
	...
	out << "CORBA::ValueFactoryBase* oldFact;\n";
	out << "oldFact = orb->register_value_factory (\"" << value->id() << "\", this);\n";
	out << "if (oldFact) DEBUG_OUT(\"factory for " << value->id() << " already registered\");\n";
	out.unindent();
	out << "}\n\n\n";
