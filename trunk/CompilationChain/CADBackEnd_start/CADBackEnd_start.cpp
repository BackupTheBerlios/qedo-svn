...
#ifdef _WIN32
#ifdef ORBACUS_ORB
	cad_servant.append("\\orbacus\CCMGenerator_CADBackEnd_compo_SERVANT.dll");
...
#else//!_WIN32
#ifdef ORBACUS_ORB
	cad_servant.append("/Debug_orbacus/libHelloWorld_CalleeImpl_SERVANT.so");
	cad_exec.append("/Debug_orbacus/libHelloWorld_CalleeImpl.so");
#endif//ORBACUS_ORB
#ifdef MICO_ORB
	cad_servant.append("/Debug_mico/libHelloWorld_CalleeImpl_SERVANT.so");
	cad_exec.append("/Debug_mico/libHelloWorld_CalleeImpl.so");
#endif//ORBACUS_ORB
#endif//_WIN32*/
	
	std::string CADBackEnd_loc = cad_servant + ";create_CADBackendHomeS;" + cad_exec + ";create_CADBackendHomeE";
	try
	{
		component_installer->install ("CADBACKEND", CADBackEnd_loc.c_str());
	}
...
main (int argc, char** argv)
{
	...
	//
	std::cout << "Test Client for CADBACKEND Container" << std::endl;
	//
	cout << "Enter \n";
	cin >> input;
	*/
	...
	home = container->install_home ("CADBACKEND", "", config);
	CCMGenerator::CADBackendHome_var cad_home = CCMGenerator::CADBackendHome::_narrow (home);
	if (CORBA::is_nil (cad_home))
	{
		std::cerr << "I got a nil reference for the install CADBACKEND home" << std::endl;
		orb->destroy();
		exit (-1);
	}

	CCMGenerator::CADBackend_var cad_backend;
	...
	if (CORBA::is_nil (cad_home))
	{
		std::cerr << "I got a nil reference for the CAD BackEnd by create" << std::endl;
		orb->destroy();
		exit (-1);
	}
	CORBA::Object_var obj_;
	obj_ = CCMGenerator::HelpFunctions::get_repository_ref ( orb );
	cad_backend->rep_ref (obj_);
// attribute setzen
	component_id = argv[1];
	target = argv[2];
	try
	{
		obj_ = cad_backend->provide_control();
		...
		std::cout << "configure CAD Backend" << std::endl;
		cad_backend->configuration_complete();
