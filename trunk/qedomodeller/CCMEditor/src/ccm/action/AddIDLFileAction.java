 ...
 * @author siegercn
 */
public class AddIDLFileAction extends SelectionAction {
	
//	The command type as string value
	private final String commandType;
	
	// The name shown in the Context-Menu
	public static final String CLASSIFIER_PROPERTY = "add a IDLFile";
	 ...
	 * @param commandType
	 */
	public AddIDLFileAction(IWorkbenchPart part, String commandType){
		super(part);
	...
	protected Command createCommand(List selectedEditParts){
		...
		if(selectedEditParts.get(0) instanceof EditPart) {
			...
			if ((part instanceof SoftwarePackageNodeEditPart 
			   && ((SoftwarePackage)((SoftwarePackageNodeEditPart)part).getModelNode().getContained()).getIdlFile()==null)
			   ||
					(part instanceof CCMModelTreeEditPart 
							&& part.getModel()instanceof SoftwarePackage
							&&((SoftwarePackage)part.getModel()).getIdlFile()==null)) {
				
				//AddStructDefRequest req = new AddStructDefRequest(getCommandType());
				//if (!part.understandsRequest(req)) return null;
				//else {
					ModelFactory mf = new ModelFactory();
					AddIDLCommand command = new AddIDLCommand();
					...
					command.setNewObject(factory.createIDLFile());
