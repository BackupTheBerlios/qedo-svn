 ...
 * @author siegercn
 */
public class RegisterComponentInstanceAction extends SelectionAction {
	
//	The command type as string value
	private final String commandType;
	
	// The name shown in the Context-Menu
	public static final String CLASSIFIER_PROPERTY = "add register component instance";
	 ...
	 * @param commandType
	 */
	public RegisterComponentInstanceAction(IWorkbenchPart part, String commandType){
		super(part);
	...
	protected Command createCommand(List selectedEditParts){
		...
		if(selectedEditParts.get(0) instanceof EditPart) {
			...
			if (part instanceof ComponentInstanceNodeEditPart ||
					(part instanceof CCMModelTreeEditPart&& (part.getModel()instanceof ComponentInstantiation))) {
				
				//AddStructDefRequest req = new AddStructDefRequest(getCommandType());
				//if (!part.understandsRequest(req)) return null;
				//else {
					ModelFactory mf = new ModelFactory();
					AddRegisterComponentInstanceCommand command = new AddRegisterComponentInstanceCommand();
