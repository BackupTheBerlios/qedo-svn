 ...
 * @author siegercn
 */
public class RuleAction extends SelectionAction {
	
//	The command type as string value
	private final String commandType;
	
	// The name shown in the Context-Menu
	public static final String CLASSIFIER_PROPERTY = "add a rule";
	 ...
	 * @param commandType
	 */
	public RuleAction(IWorkbenchPart part, String commandType){
		super(part);
	...
	protected Command createCommand(List selectedEditParts){
		...
		if(selectedEditParts.get(0) instanceof EditPart) {
			...
			if (part instanceof AssemblyNodeEditPart ||part instanceof HomeInstanceNodeEditPart||
					...
					(part instanceof CCMModelTreeEditPart&& (part.getModel()instanceof HomeInstantiation
							||part.getModel()instanceof Assembly
							||part.getModel()instanceof ComponentInstantiation))) {
				...
				//else {
					...
					command.setNewObject(factory.createRule());
