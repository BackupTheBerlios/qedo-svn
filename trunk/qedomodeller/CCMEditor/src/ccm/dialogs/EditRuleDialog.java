 ...
 * @author Siegercn
*/
public class EditRuleDialog extends org.eclipse.swt.widgets.Dialog implements KeyListener{
	...
	public EditRuleDialog(Shell parent, int style) {
		super(parent, style);
	}
	...
	*/
	public void open(){
		try {
			...
			dialogShell.setText("Edit Rules");
	...
	* However, if you delete this method it will not be re-created.	*/
	public static void showGUI(){
		try {
			...
			EditRuleDialog inst = new EditRuleDialog(shell, SWT.NULL);
			inst.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void addParameter(){
		 CreateRuleDialog d=new CreateRuleDialog(this.dialogShell, 0);
	   ...
	   rule.setIdentifier(d.getIdentifier());
	   rule.setVersion(d.getVersion());
	   rule.setRepositoryId(d.getRepositoryId());
	   rule.setName(d.getIdentifier());
	   rule.setLanguage(d.getLanguage());
	   rule.setCondition(d.getCondition());
	   rule.setAction(d.getAction());
	    
	    compFileList.add(rule);
	    par.add(rule.getIdentifier());
	    if(par.getItemCount()>0){
	...
	private void editParameter(){
		 int index=par.getSelectionIndex();
		    if(index==-1)return;
		    Rule rule=(Rule) compFileList.get(index);
		    CreateRuleDialog d=new CreateRuleDialog(this.dialogShell, 0);
		    d.setIdentifier(rule.getIdentifier());
		    d.setRepositoryID(rule.getRepositoryId());
		    d.setVersion(rule.getVersion());
		    d.setCondition(rule.getCondition());
			d.setLanguage(rule.getLanguage());
		    d.setAction(rule.getAction());
		  ...
		  // Rule rule=factory.createRule();
		   ...
		   rule.setCondition(d.getCondition());
		   rule.setAction(d.getAction());
		   compFileList.set(index,rule);
		   par.setItem(index,rule.getIdentifier());
		}
