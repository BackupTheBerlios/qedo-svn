...
*@author Siegercn
*/
public class EditExceptionsofOperationDialog extends org.eclipse.swt.widgets.Dialog  implements KeyListener{
	...
	private java.util.List poss_exceptionList=new LinkedList();
	

	public EditExceptionsofOperationDialog(Shell parent, int style) {
		super(parent, style);
	}
	...
	protected void addPMouseDown(MouseEvent evt){
		AddExceptionDialog d=new AddExceptionDialog(this.dialogShell,0,"Add Enum");
	    ExceptionDef exc = null;
	    d.setExcList(this.poss_exceptionList);
	    d.open();
	    if(!d.isState())return;
	    exc=d.getField();
	    memberList.add(exc);
	    par.add(exc.getIdentifier());
	    if(par.getItemCount()>0){
	...
	*/
	public void open(){
		try {
			...
			dialogShell.setText("Edit Exceptions");
	...
	public void postInitGUI(){
	    ...
	    while(it.hasNext()){
	        par.add( ((ExceptionDef)it.next()).getIdentifier());
	    }
	    if(par.getItemCount()==0){
	        removeP.setEnabled(false);
	    }else{
	...
	* However, if you delete this method it will not be re-created.	*/
    ...
    public void set_possible_excpetions(java.util.List poss_exceptions)
    {
        this.poss_exceptionList.clear();
        this.poss_exceptionList.addAll(poss_exceptions);
    }
    /**
