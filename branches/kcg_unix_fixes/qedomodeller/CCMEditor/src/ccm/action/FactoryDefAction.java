 ...
 * @author eduardw
 */
public class FactoryDefAction extends SelectionAction {
	
//	The command type as string value
	private final String commandType;
	
	// The name shown in the Context-Menu
	public static final String CLASSIFIER_PROPERTY = "add factory";
	 ...
	 * @param commandType
	 */
	public FactoryDefAction(IWorkbenchPart part, String commandType){
		super(part);
	...
	{
		Command cmd = createFactoryDefCommand(getSelectedObjects());
		if (cmd == null)
			return false; 
	 ...
	 */
	protected Command createFactoryDefCommand(List selectedEditParts) {
		...
		if(selectedEditParts.get(0) instanceof EditPart) {
			...
			if (!(part instanceof HomeDefNodeEditPart) &&
					!(part instanceof CCMModelTreeEditPart)) return null;
			
			AddFactoryRequest req = new AddFactoryRequest(getCommandType());
			if (!part.understandsRequest(req)) return null;
			
			ModelFactory mf = new ModelFactory();
			AddOperationDefCommand command = (AddOperationDefCommand) part.getCommand(req);
			...
			command.setNewObject(factory.createFactoryDef());
	 ...
	 */
	public void run(){
		execute(createFactoryDefCommand(getSelectedObjects()));
	}
