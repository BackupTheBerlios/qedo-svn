 ...
 * @author siegercn
 */
public class AddImplementationAction extends SelectionAction {
	
//	The command type as string value
	private final String commandType;
	
	// The name shown in the Context-Menu
	public static final String CLASSIFIER_PROPERTY = "add a Implementation";
	 ...
	 * @param commandType
	 */
	public AddImplementationAction(IWorkbenchPart part, String commandType){
		super(part);
	...
	protected Command createCommand(List selectedEditParts){
		...
		if(selectedEditParts.get(0) instanceof EditPart) {
			...
			if (part instanceof SoftwarePackageNodeEditPart ||
					(part instanceof CCMModelTreeEditPart&& (part.getModel()instanceof SoftwarePackage))) {
				
				//AddStructDefRequest req = new AddStructDefRequest(getCommandType());
				//if (!part.understandsRequest(req)) return null;
				//else {
					ModelFactory mf = new ModelFactory();
					CreateImplementationCommand command = new CreateImplementationCommand();
					...
					command.setNewObject(factory.createImplementation());
