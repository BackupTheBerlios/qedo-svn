 ...
 * @author eduardw
 */
public class ValueMemberDefAction extends SelectionAction {
	
//	The command type as string value
	private final String commandType;
	
	// The name shown in the Context-Menu
	public static final String CLASSIFIER_PROPERTY = "add valueMember";
	 ...
	 * @param commandType
	 */
	public ValueMemberDefAction(IWorkbenchPart part, String commandType){
		super(part);
	...
	protected Command createOperatonCommand(List selectedEditParts) {
		...
		if(selectedEditParts.get(0) instanceof EditPart) {
			...
			if (part instanceof ValueDefNodeEditPart ||
					part instanceof CCMModelTreeEditPart) {
				
				AddValueMembertDefRequest req = new AddValueMembertDefRequest(getCommandType());
				if (!part.understandsRequest(req)) return null;
				else {
					ModelFactory mf = new ModelFactory();
					AddValueMemberDefCommand command = 
						(AddValueMemberDefCommand) part.getCommand(req);
					...
					command.setNewObject(factory.createValueMemberDef());
