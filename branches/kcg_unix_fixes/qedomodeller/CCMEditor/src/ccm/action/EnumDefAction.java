 ...
 * @author eduardw
 */
public class EnumDefAction extends SelectionAction {
...
//	The command type as string value
	 ...
	 */
	public static final String CLASSIFIER_PROPERTY = "add enum";
	 ...
	 * @param commandType
	 */
	public EnumDefAction(IWorkbenchPart part, String commandType){
		super(part);
	 ...
	 * a command for this action can be instanciated (AddNodeForEnumDefCommand)
	 * and the command can be executed (cmd.canExecute()). 
	 */
	protected boolean calculateEnabled()
	{
		Command cmd = createEnumCommand(getSelectedObjects());
		if (cmd == null)
			return false; 
	 ...
	 */
	protected Command createEnumCommand(List selectedEditParts){
		...
		if(selectedEditParts.get(0) instanceof EditPart) {
			...
			if (part instanceof InterfaceDefNodeEditPart ||
				...
				AddEnumDefRequest req = new AddEnumDefRequest(getCommandType());
				if (!part.understandsRequest(req)) return null;
				else {
					ModelFactory mf = new ModelFactory();
					AddEnumDefCommand command = 
						(AddEnumDefCommand) part.getCommand(req);
					...
					command.setNewObject(factory.createEnumDef());
	 ...
	 */
	public void run(){
		execute(createEnumCommand(getSelectedObjects()));
	}
