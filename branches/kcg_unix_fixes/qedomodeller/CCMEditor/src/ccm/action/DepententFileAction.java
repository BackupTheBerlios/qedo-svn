 ...
 * @author siegercn
 */
public class DepententFileAction extends SelectionAction {
	
//	The command type as string value
	private final String commandType;
	
	// The name shown in the Context-Menu
	public static final String CLASSIFIER_PROPERTY = "add dependent file";
	 ...
	 * @param commandType
	 */
	public DepententFileAction(IWorkbenchPart part, String commandType){
		super(part);
		...
		if(selectedEditParts.get(0) instanceof EditPart) {
			...
			if (part instanceof DeploymentUnitNodeEditPart ||
				...
				//else {
					...
					AddDepententFileCommand command = new AddDepententFileCommand();
				...
				//	command.setIdlTypes(mf.getIDLTypes(command.getRootModule()));
					command.setNewObject(factory.createDependentFile());
