...
GeneratorServantC::generate(std::string target, std::string fileprefix)
{
	...
	/* disabled only for the test under Qedo: br_0_1_5_wang
	  it should be recovered in merging in main branche */
	//out << "#include \"CDRTransportCoDec.h\"\n";
	//out << "#include \"MarshalBuffer.h\"\n";
	/* end: disabled only for the test under Qedo: br_0_1_5_wang */
...
GeneratorServantC::genContextServant(IR__::ComponentDef_ptr component)
{
	...
	for( i= 0; i < len; i++)
	{
		...
		else
		{
			...
			if(composition_->lifecycle()==CIDL::lc_Entity || composition_->lifecycle()==CIDL::lc_Process)
			{
				out << "StorageObject* ccm_obj = dynamic_cast <StorageObject*> (ccm_obj_);\n";
				out << strNamespace_ << "::" << component_->name() << "Persistence* " << "ccm_object = dynamic_cast <" << strNamespace_ << "::" << component_->name() << "Persistence*> (ccm_obj);\n\n";
				out << "#ifdef MICO_ORB\n";
				out << "CORBA::ORB_var orb = CORBA::ORB_instance (\"mico-local-orb\");\n";
				out << "#endif\n";
				out << "ccm_object->" << a_uses->name() << "(orb->object_to_string(use));\n";
			}
			out << "return use._retn();\n";
			out.unindent();
			out << "}\n\n\n";
	...
	for(i = 0; i < len; i++) 
	{
		...
		if(composition_->lifecycle()==CIDL::lc_Entity || composition_->lifecycle()==CIDL::lc_Process)
		{
			...
			out << "CORBA::ORB_var orb = CORBA::ORB_instance (\"mico-local-orb\");\n";
			out << "#endif\n";
			out << "ccm_object->" << a_emits->name() << "(orb->object_to_string(ev));\n";
		}*/
		out.unindent();
		out << "}\n";
		out.unindent();
	...
	for(i = 0; i < len; i++) 
	{
		...
		if(composition_->lifecycle()==CIDL::lc_Entity || composition_->lifecycle()==CIDL::lc_Process)
		{
			...
			out << "CORBA::ORB_var orb = CORBA::ORB_instance (\"mico-local-orb\");\n";
			out << "#endif\n";
			out << "ccm_object->" << a_publishes->name() << "(orb->object_to_string(ev));\n";
		}*/
		out.unindent();
		out << "}\n\n\n";
	}
...
GeneratorServantC::genHomeServantBegin(IR__::HomeDef_ptr home, CIDL::LifecycleCategory lc)
{
	...
	*/
	if(composition_->lifecycle()==CIDL::lc_Entity || composition_->lifecycle()==CIDL::lc_Process)
	{
		...
		out << ");\n";
		// check whether component has consumes
		IR__::ContainedSeq_var contained_seq = comp->contents(CORBA__::dk_Consumes, false);
		if(contained_seq->length()>0)
		{	
			out << "#ifdef MICO_ORB\n";
			out << "CORBA::ORB_var orb = CORBA::ORB_instance (\"mico-local-orb\");\n";
			out << "#endif\n";

			for( unsigned int cnt=0; cnt<contained_seq->length(); cnt++ )
			{
				IR__::ConsumesDef_var a_consumes = IR__::ConsumesDef::_narrow(((*contained_seq)[cnt]));
				out << "pCcmStorageObject->" << a_consumes->name() << "(orb->object_to_string(" << a_consumes->name() << "_ref));\n";
			}
		}
...
GeneratorServantC::genTableForAbsStorageHome()
{
	...
	for( CORBA::ULong i=0; i<supported_infs->length(); i++ )
	{
		...
		{
			strContent = "pid  VARCHAR(512)  NOT NULL  REFERENCES PID_CONTENT";
			out << genSQLLine(strName, strContent, true, true, true);
			strContent = "spid  VARCHAR(512)";
			out << genSQLLine(strName, strContent, true, true, true);
		}
...
GeneratorServantC::genTableForHome(IR__::HomeDef_ptr home)
{
	...
	for( i = 0; i < contained_seq->length(); i++ )
	{
		...
		required.push_back(a_consumes->name());
	}
