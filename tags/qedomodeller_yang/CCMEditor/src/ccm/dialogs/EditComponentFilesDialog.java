 ...
 * @author Siegercn
*/
public class EditComponentFilesDialog extends org.eclipse.swt.widgets.Dialog implements KeyListener{
	...
	private java.util.List rules=new LinkedList();
	...
	public EditComponentFilesDialog(Shell parent, int style) {
		super(parent, style);
	}
	...
	*/
	public void open(){
		try {
			...
			dialogShell.setText("Edit ComponentFiles");
	...
	public void postInitGUI(){
	    ...
	    while(it.hasNext()){
	        ComponentFile parameter=(ComponentFile) it.next();
	...
	* However, if you delete this method it will not be re-created.	*/
	public static void showGUI(){
		try {
			...
			EditComponentFilesDialog inst = new EditComponentFilesDialog(shell, SWT.NULL);
			inst.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void editParameter(){
		 int index=par.getSelectionIndex();
		    if(index==-1)return;
		    ComponentFile file=(ComponentFile) rules.get(index);
		    CreateComponentFileDialog d=new CreateComponentFileDialog(this.dialogShell,0);
