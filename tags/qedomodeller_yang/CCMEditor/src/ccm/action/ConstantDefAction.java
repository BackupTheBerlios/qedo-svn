 ...
 * @author eduardw
 */
public class ConstantDefAction extends SelectionAction {
...
//	The command type as string value
	 ...
	 */
	public static final String CLASSIFIER_PROPERTY = "add constant";
	 ...
	 * @param commandType
	 */
	public ConstantDefAction(IWorkbenchPart part, String commandType){
		super(part);
	 ...
	 * a command for this action can be instanciated (AddNodeForConstantDefCommand)
	 * and the command can be executed (cmd.canExecute()). 
	 */
	protected boolean calculateEnabled()
	{
		Command cmd = createConstantCommand(getSelectedObjects());
		if (cmd == null)
			return false; 
	 ...
	 */
	protected Command createConstantCommand(List selectedEditParts){
		...
		if(selectedEditParts.get(0) instanceof EditPart) {
			...
			if (part instanceof InterfaceDefNodeEditPart ||
				...
				AddConstantDefRequest req = new AddConstantDefRequest(getCommandType());
				if (!part.understandsRequest(req)) return null;
				else {
					ModelFactory mf = new ModelFactory();
					AddConstantDefCommand command = 
						(AddConstantDefCommand) part.getCommand(req);
					...
					command.setNewObject(factory.createConstantDef());
	 ...
	 */
	public void run(){
		execute(createConstantCommand(getSelectedObjects()));
	}
