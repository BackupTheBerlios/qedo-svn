 ...
 */
public class AddEventSinkAction extends SelectionAction {
	
	//	 The command type as string value
	private final String commandType;
	
	// The name shown in the Context-Menu
	public static final String CLASSIFIER_PROPERTY = "add event sink";
	 ...
	 * @param commandType
	 */
	public AddEventSinkAction(IWorkbenchPart part, String commandType){
		super(part);
	...
	{
		Command cmd = createEventSinkCommand(getSelectedObjects());
		if (cmd == null)
			return false; 
	 ...
	 */
	protected Command createEventSinkCommand(List selectedEditParts)
	{
        ...
        AddEventSinkRequest req = new AddEventSinkRequest(getCommandType());
    	if (!part.understandsRequest(req)) return null;

    	ModelFactory mf = new ModelFactory();
    	AddEventSinkCommand command = (AddEventSinkCommand) part.getCommand(req);
	...
	public void run()
	{
		execute(createEventSinkCommand(getSelectedObjects()));
	}
