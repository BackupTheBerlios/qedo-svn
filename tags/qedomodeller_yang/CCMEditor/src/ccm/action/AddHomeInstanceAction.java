...
import ccm.edit.ProcessCollocationNodeEditPart;
 ...
 * @author siegercn
 */
public class AddHomeInstanceAction extends SelectionAction {
	
//	The command type as string value
	private final String commandType;
	
	// The name shown in the Context-Menu
	public static final String CLASSIFIER_PROPERTY = "add a Home Instance";
	 ...
	 * @param commandType
	 */
	public AddHomeInstanceAction(IWorkbenchPart part, String commandType){
		super(part);
	...
	protected Command createCommand(List selectedEditParts){
		...
		if(selectedEditParts.get(0) instanceof EditPart) {
			...
			if (part instanceof ProcessCollocationNodeEditPart ||
					(part instanceof CCMModelTreeEditPart&& (part.getModel()instanceof ProcessCollocation))) {
				
				//AddStructDefRequest req = new AddStructDefRequest(getCommandType());
				//if (!part.understandsRequest(req)) return null;
				//else {
					ModelFactory mf = new ModelFactory();
					CreateHomeInstanceCommand command = new CreateHomeInstanceCommand();
					...
					command.setNewObject(factory.createHomeInstantiation());
