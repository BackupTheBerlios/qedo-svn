 ...
 * @author eduardw
 */
public class ValueBoxDefAction extends SelectionAction {
	
//	The command type as string value
	private final String commandType;
	
	// The name shown in the Context-Menu
	public static final String CLASSIFIER_PROPERTY = "add value box";
	 ...
	 * @param commandType
	 */
	public ValueBoxDefAction(IWorkbenchPart part, String commandType){
		super(part);
	...
	protected Command createOperatonCommand(List selectedEditParts){
		...
		if(selectedEditParts.get(0) instanceof EditPart) {
			...
			if (part instanceof InterfaceDefNodeEditPart ||
				...
				AddValueBoxDefRequest req = new AddValueBoxDefRequest(getCommandType());
				if (!part.understandsRequest(req)) return null;
				else {
					ModelFactory mf = new ModelFactory();
					AddValueBoxDefCommand command = 
						(AddValueBoxDefCommand) part.getCommand(req);
					...
					command.setNewObject(factory.createValueBoxDef());
